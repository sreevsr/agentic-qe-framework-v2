...
Selectors come from Scout (user-verified element discovery) and code was generated by the Builder from those verified selectors. If tests fail, the cause is almost certainly timing, sequencing, or environment — NOT wrong selectors. If selectors ARE wrong, it means Scout missed the element — escalate for Scout re-scan, do NOT guess new selectors.
...
- If correct page but element missing → escalate to Scout/Builder for re-exploration
...
| Issue | Signal | Action |
|-------|--------|--------|
| Wrong selector | Element not found AND not in DOM snapshot | **STOP.** Escalate to Scout/Builder for re-exploration |
...
| Architecture issue | Page object or locator file missing | **STOP.** Scout/Builder output incomplete. Escalate |
